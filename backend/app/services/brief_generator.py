...
class BriefGenerationService:
    ...
    def generate_bundle(self, extracted: ExtractedCaseDraft, owner_alias: str, difficulty: str, estimated_minutes: int) -> AuthoringBundle:
        ...
        rescan_rules = self._build_rescan_rules(extracted, suspect_configs, documents)
        ...
        case_config = CaseConfig(
            ...
            location_dossiers=self._build_locations(extracted, documents),
        ...
        for index, document in enumerate(hidden_documents, start=1):
            ...
            rules.append(
                RescanRule(
                    ...
                    trigger=Trigger(type="context_entity_discovered", value=trigger_value),
                    effects=TriggerEffects(unlock_document_ids=[document.id], surface_document_ids=["doc_incident"]),
                )
            )

        locked_suspects = [suspect for suspect in suspects if suspect.unlock_rule]
        if locked_suspects and hidden_documents:
            rules.append(
                RescanRule(
                    id=f"unlock_{locked_suspects[0].id}",
                    trigger=Trigger(type="board_link_confirmed", value=f"victim-{hidden_documents[0].id}-hidden-thread"),
                    effects=TriggerEffects(unlock_suspect_ids=[locked_suspects[0].id]),
                )
            )
        return rules
